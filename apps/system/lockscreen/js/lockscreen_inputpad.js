...
(function(exports) {
    ...
    while (i--) {
      ...
      if (span) {
        if (this.states.passCodeEntered.length > i) {
          ...
          if ((this.states.passCodeEntered.length - 1 === i) && isPassCodeEntered) {
            span.classList.add('current-passcode-border');
            this.startPassCodeTimeout(span, value);
          } else {
            if(!isPassCodeEntered && this.states.passCodeEntered.length - 1 === i) {
              span.classList.add('current-passcode-border');
            } else {
  ...
  LockScreenInputpad.prototype.handleKeyEvent = function(event) {
    ...
    // operation and keycode for 'e' is 69 and used it to make the emergency call.
    // If user pressed 'e' button for making emergency call, isEmergencyEnabled
    // should be true.
    // this code will be modified when it is integerated with the actual keyboard.
